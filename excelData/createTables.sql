...
CREATE TABLE HasSideEffect(drugbank_id VARCHAR(255) NOT NULL, umls_cui VARCHAR(255) NOT NULL,PRIMARY KEY (drugbank_id,umls_cui),FOREIGN KEY(drugbank_id) REFERENCES DrugBanks(drugbank_id) ,FOREIGN KEY(umls_cui) REFERENCES SIDERS(umls_cui) ON DELETE CASCADE);
CREATE TABLE Papers(doi VARCHAR(255) NOT NULL, institution VARCHAR(255), PRIMARY KEY (doi));
CREATE TABLE Reactions(reaction_id VARCHAR(255) NOT NULL,uniprot_id VARCHAR(255) NOT NULL,drugbank_id VARCHAR(255) NOT NULL,doi VARCHAR(255) NOT NULL,measure_name VARCHAR(255),affinity_nM  REAL,target_name VARCHAR(255),smiles VARCHAR(255), PRIMARY KEY (reaction_id), FOREIGN KEY (uniprot_id) REFERENCES UniProts(uniprot_id) ,FOREIGN KEY (drugbank_id) REFERENCES DrugBanks(drugbank_id) , CONSTRAINT MeasureName CHECK (measure_name = 'Ki' OR measure_name ='Kd' OR measure_name ='IC50'));
CREATE TABLE InteractsWith(drugbank_id_1 VARCHAR(255) NOT NULL,drugbank_id_2 VARCHAR(255) NOT NULL,PRIMARY KEY (drugbank_id_1 , drugbank_id_2), FOREIGN KEY (drugbank_id_1) REFERENCES DrugBanks(drugbank_id) ,FOREIGN KEY (drugbank_id_2) REFERENCES DrugBanks(drugbank_id) )
...
CREATE TRIGGER delete_drug_records_reactions BEFORE DELETE ON dtbank.drugbanks FOR EACH ROW BEGIN DELETE FROM dtbank.reactions WHERE dtbank.reactions.drugbank_id = OLD.drugbank_id; END;
CREATE TRIGGER delete_drug_records_hassideeffect BEFORE DELETE ON dtbank.drugbanks FOR EACH ROW BEGIN DELETE FROM dtbank.hassideeffect WHERE dtbank.hassideeffect.drugbank_id = OLD.drugbank_id; END;
CREATE TRIGGER delete_drug_records_interactswith BEFORE DELETE ON dtbank.drugbanks FOR EACH ROW BEGIN DELETE FROM dtbank.interactswith WHERE ((dtbank.interactswith.drugbank_id_1 = OLD.drugbank_id and drugbank_id_2 <> '-1') or (dtbank.interactswith.drugbank_id_2 = OLD.drugbank_id and drugbank_id_1 <> '-1')); END;
CREATE TRIGGER delete_protein_records_reactions BEFORE DELETE ON dtbank.uniprots FOR EACH ROW BEGIN DELETE FROM dtbank.reactions WHERE dtbank.reactions.uniprot_id = OLD.uniprot_id; END;