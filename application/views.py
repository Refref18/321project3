...
def user():
    ...
    if request.method == 'POST':
        ...
        if request.form.get('action2') == 'Drug Interaction Table':
            ...
            mycursor.execute(
                "SELECT I.drugbank_id_2 as interacting_drug, D2.name FROM InteractsWith I , Drugbanks D, Drugbanks D2 WHERE I.drugbank_id_1=D.drugbank_id and I.drugbank_id_2=D2.drugbank_id and D.drugbank_id=%s ", (drugbank_id,))
            columns = [col[0] for col in mycursor.description]
            rows = [dict(zip(columns, row)) for row in mycursor.fetchall()]
            print(rows)
            rowdict = {}
            for row in rows:
                rowdict[row['interacting_drug']] = row
            # input: drugbank_id
    ...
    if request.method == 'POST':
        ...
        if request.form.get('action4.2') == 'Add Contributor':
            ...
            try:
                ...
                mycursor.execute(
                    "SELECT * FROM Papers WHERE doi= %s and institution = %s", (
                        doi, ins)
                )
                m = mycursor.fetchall()
                print(m)
